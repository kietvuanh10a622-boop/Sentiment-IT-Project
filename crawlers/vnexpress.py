...
class VnExpressCrawler(BaseCrawler):
    """Crawler chuyên biệt cho VnExpress với phạm vi đa ngành rộng hơn."""

    ROUTES = [
        ('Technology', ['/cong-nghe', '/khoa-hoc', '/suc-khoe']),
        ('Business', ['/kinh-doanh', '/thi-truong', '/tai-chinh']),
        ('Macro Economy', ['/kinh-doanh', '/thi-truong', '/tai-chinh']),
        ('World', ['/the-gioi', '/quoc-te', '/thoi-su']),
        ('Science', ['/khoa-hoc', '/suc-khoe', '/cong-nghe']),
        ('Real Estate', ['/bat-dong-san', '/kinh-doanh']),
    ]

    def __init__(self):
        super().__init__(base_url='https://vnexpress.net', source_name='VnExpress')

    def crawl_articles(self, max_articles=400):
    ...
    def fetch_route_articles(self, category, url, page):
        try:
            ...
            logging.warning(f'VnExpress fetch error for {url}: {exc}')
            return []
